...
def check_version(version):
    try:
        link = urllib.request.urlopen(
            ...
            "/blob/master/amplicon_sorter.py"
        ).read()
        # find the version-date part of the last version on the webpage
        datepart = re.compile(
            r"(version</span>.*?)(\d{4}-\d{2}-\d{2})(.*version of the script)"
        )
        ...
        if version < latest_version:
            ...
            urllib.request.urlopen(
                "https://raw.githubusercontent.com/avierstr/"
...
def get_arguments():
    def range_limited_float_type(arg):
        ...
        if f < 50 or f > 100:
            raise argparse.ArgumentTypeError(
                "Argument must be > " + str(50.0) + " and < " + str(100.0)
            )
        return f
    ...
    def range_limited_float_0_200(arg):
        ...
        if f < 0 or f > 200:
            raise argparse.ArgumentTypeError(
                "Argument must be > " + str(0.0) + " and < " + str(200.0)
            )
        return f
    ...
    def valid_file(param):
        ...
        if os.path.isfile(param):  # if input is file
            ...
            if ext.lower() not in (".fasta", ".fastq"):
                raise argparse.ArgumentTypeError(
                    "File extension must be .fastq or .fasta"
                )
            ...
            with os.scandir(param) as iterator:
                for file in iterator:
                    if file.name.lower().endswith(
                        ".fastq"
                    ) or file.name.lower().endswith(".fasta"):
                        paramlist.append(file.path)
            paramlist.sort()
            if len(paramlist) == 0:
                sys.exit(
                    "Can not find files in folder to process.  File extension must be .fastq or .fasta"
                )
        else:
            sys.exit(
                "Can not find a file or folder to process.  File extension must be .fastq or .fasta"
            )
        param = paramlist
        return param
    ...
    parser = argparse.ArgumentParser(
        description="AmpliconSorter: Sort amplicons\
    ...
    parser.add_argument(
        "-ldc",
        "--length_diff_consensus",
        type=range_limited_float_0_200,
        required=False,
        default=8.0,
...
def save_arguments():  # save all settings in the result.txt file
    ...
    with open(os.path.join(outputfolder, "results.txt"), "a") as rf:
        ...
        rf.write(
            "- similar_species_groups = " + str(args.similar_species_groups) + "\n"
        )
        rf.write("- similar_species = " + str(args.similar_species) + "\n")
        rf.write("- similar_consensus = " + str(args.similar_consensus) + "\n")
        rf.write(
            "- length_difference_consensus = " + str(args.length_diff_consensus) + "\n"
        )
...
def ambiguity(templist, c):  # search for ambiguis bases
    ...
    for x in templist[1:]:
        if x[0][0] == ambiguitylist[0][0][0]:  # if base is the same as the previous one
            ...
            # ambiguitylist.sort(key=lambda x: int(x[0][1]), reverse=True)
            if (
                c * 0.35 <= x[0][1] <= c * 0.65 and len(ambiguitylist) < 4
            ):  # if the new base is around 50%
    ...
    for treshold in [0.45, 0.15, 0.5]:
        ...
        for y in range(len(alignlist[0])):  # for every position in alignment
            ...
            if len(templist) > 0:
                ...
                if (
                    templist[0][1] > c * 0.10
                ):  # if the first base is more than 10% present
                    templist2.append(
                        templist[0:2]
                    )  # add the 2 most abundant bases to the list
        templist2 = homopolymersort(
            templist2
        )  # sort bases in homopolymer region based on base counts
        for x in templist2:
            if x[0][1] > c * treshold:
        ...
        try:
            if x[0][0] == templist2[n - 1][0][0]:  # if the previous base is the same
                if x[0][0] in ["A", "T"]:
                    ...
                    if b >= 3:
                        if (
                            templist2[n - 1][0][1] * 0.5 < x[0][1] > c * 0.2
                        ):  # drop can not be more than half of previous base
                            consenlist.append(x[0][0])
                            b += 1
                    else:
                        if x[0][1] > c * treshold:
...
def figure(readlengthlist, total_num_seq):
    ...
    outputfolder = args.outputfolder
    figname = infile.replace(".fastq", "_total_outputfig.pdf").replace(
        ".fasta", "_total_outputfig.pdf"
    )
    ...
    plt.text(
        ...
        + "\nMax readlength: "
        + str(round(max_readlength / 1000, 1))
        + " Kb"
...
def read_file(self):  # read the inputfile
    ...
    if (
        total_num_seq < 5
    ):  # system hangs with only one sequence, less than 5 has no result
        ...
        if ran == True:
            if comp_all == True:
                if maxreads > total_num_seq:
                    print(
                        "Comparing all reads (-a, -all) with each other is not "
            ...
            for position in range(position, len(d) - 1):
                ...
                for position2 in range(position + 1, len(d)):
                    ...
                    if len(A1[1]) * 1.05 < len(A2[1]):
                        ...
                        tl += 1
                        if l == chunk:
                            todofilename = os.path.join(
                                outputfolder, "file_" + str(k) + ".todo"
                            )
    ...
    def feeder():  # function to feed the queue in parts to save memory
        ...
        while len_todolist == 0:  # start feeding when it is still making todolist
            ...
            for dirpath, dirnames, filenames in os.walk(outputfolder):
                filenames = [i for i in filenames if i.endswith(".todo")]
                filenames.sort(
                    key=lambda x: os.path.getmtime(os.path.join(outputfolder, x))
...
def finetune(grouplist):
    ...
    for i, group in enumerate(grouplist):
        ...
        consensus1 = scorelist[int(len(scorelist) // 1.25)][
            1
        ]  # take seq somewhere at the end
        consensus2 = scorelist[int(len(scorelist) // 5)][
            1
        ]  # take seq somewhere at the begin
...
def update_list(tempfile):  # create gene-groups from compared sequences
    ...
    for j, x in enumerate(grouplist):
        outputfile = os.path.join(
            outputfolder,
            infile.replace(".fastq", "_").replace(".fasta", "_") + str(j) + ".group",
        )
...
def make_consensus(todoqueue, outputfolder, consensus_tempfile, infile):
    ...
    for X in iter(todoqueue.get, "STOP"):  # do stuff until infile.get returns "STOP"
        for i, x in X:
            ...
            consensus, alignlist = create_consensus(
                consensuslist2, infile
            )  # create consensus sequence
            templist.append([i, consensus])
            if aln is True:  # option to write alignment to file
                try:  # remove temporary alignment files if exists
                    filename = os.path.join(outputfolder, infile).replace(
                        ".group", "*_alignment.fasta"
                    )
                ...
                if infile != "_":
                    outfile = (
                        os.path.join(outputfolder, infile).replace(".group", "_")
                        + str(i)
                        + "_alignment.fasta"
                    )
                    with open(outfile, "w") as alignm:
...
def comp_consensus_groups(grouplist):  # compare consensuses with each other
    ...
    nprocesses = args.nprocesses
    length_diff_c = (
        args.length_diff_consensus / 100 + 1
    )  # length difference allowed for
    # consensus
    ...
    while a1 > a2:  # limit number of cycles
        ...
        do_parallel(
            outputfolder, nprocesses, consensus_tempfile, make_consensus, stringx, "_"
        )
        ...
        try:
            ...
            for line in temp:
                ...
                grouplist[int(i)].append(
                    consensus
                )  # add consensus sequence to the group
            os.remove(os.path.join(outputfolder, "consensus.tmp"))
        except FileNotFoundError:
            pass
        # ---------------------------------------------------------------------
        ...
        for position in range(position, len(grouplist) - 1):
            ...
            for position2 in range(position + 1, len(grouplist)):
                ...
                if len(A1) * length_diff_c < len(A2) or len(A2) * length_diff_c < len(
                    A1
                ):
                    pass  # don't compare if length of sequences differ to much
                else:
        ...
        do_parallel(
            outputfolder, nprocesses, consensus_tempfile, iden_consensus, stringx, "_"
        )
        ...
        try:
            ...
            for line in temp:
                ...
                if (
                    length_diff_c > 1.08
                ):  # for larger length diff: lot of groups can be merged...
                    if float(iden) >= 0.80:  # ... if iden > 0.6
                        while (
                            len(grouplist[int(y)]) == 1
                        ):  # check if groups points to an other group
                            y = int(
                                list(grouplist[int(y)])[0].replace("=", "")
                            )  # replace y to the other group
    ...
    a2 = len(grouplist)
    print(
        "--> Number of groups after removing groups with less than 5 sequences: "
        + str(a2)
    )
...
def read_indexes(group_filename):  # read index numbers from the the group file
    ...
    if group_filename.endswith("nogroup.group"):
        ...
        a2 = len(grouplist)
        print(
            "--> Number of groups after removing groups with less than 4 sequences: "
            + str(a2)
        )
...
def filter_seq(group_filename, grouplist, indexes):
    ...
    for rec_id, seq, scores, index in comparelist2:
        if str(index) in indexes:
            if scores == "u":  # sequences that have no similarity with others
                outputfile = (
                    os.path.join(outputfolder, group_filename).replace(".group", "_")
                    + "unique.fasta"
                )  # unique sequences
                outputfilefq = (
                    os.path.join(outputfolder, group_filename).replace(".group", "_")
    ...
    consensusfilename = os.path.join(outputfolder, group_filename).replace(
        ".group", "_consensussequences.fasta"
    )  # group consensusfile
    # total consensusfile
    consensusfile = (
        os.path.join(outputfolder, infile)
        .replace(".fasta", "_consensussequences.fasta")
        .replace(".fastq", "_consensussequences.fasta")
    )
        ...
        with open(os.path.join(outputfolder, "results.txt"), "a") as rf:
            try:
                rf.write(
                    ...
                    + str(t)
                    + " sequences ("
                    + str(round(t * 100 / len(comparelist2), 2))
                    + "% of total)\n"
                )
        ...
        try:
            if group_filename.endswith("nogroup.group"):
                rf.write(
                    ...
                    + str(os.path.split(outputfile)[1])
                    + " ("
                    + str(round(len(indexes) * 100 / len(comparelist2), 2))
                    + "% of total)\n"
                )
            else:
                rf.write(
...
def process_consensuslist(indexes, grouplist, group_filename):
    ...
    outputfolder = args.outputfolder
    nprocesses = args.nprocesses
    group_tempfile = os.path.join(outputfolder, group_filename).replace(
        ".group", ".tmp"
    )
    try:  # remove temporary file if exists
    ...
    if len(todolist) > 0:
        stringx = "...processing: "
        do_parallel(
            outputfolder, nprocesses, group_tempfile, similarity_species, stringx, "_"
        )
    ...
    for X in iter(todoqueue.get, "STOP"):  # do stuff until infile.get returns "STOP"
        for A1, A2 in X:
            try:
                ...
                if (
                    iden >= similar - 0.01
                ):  # -0.01 to speed up if no seq passed criteria
                    templist.append((str(A1[3]) + ":" + str(A2[0]) + ":" + str(iden)))
                elif iden < 0.5:
                    iden = distance(A1[1], compl_reverse(A2[1]))
                    if iden >= similar - 0.01:
                        templist.append(
                            (str(A1[3]) + ":" + str(A2[0]) + ":" + str(iden))
                        )
            except KeyboardInterrupt:
                print("Shutting process down")
    MYLOCK.acquire()
    ...
    if len(templist2) == 0:
        ...
        for i, x in enumerate(grouplist):
            if x[-1].isdigit():  # if the last item is a number, sequence has been added
                if (
                    len(x) > 200
                ):  # if number of reads > 200, only take 200 for consensus
        ...
        while a1 > a2 and b < 3:  # limit number of cycles
            ...
            for position in range(position, len(grouplist) - 1):
                ...
                for position2 in range(position + 1, len(grouplist)):
                    ...
                    A1 = grouplist[position][-1]
                    A2 = grouplist[position2][-1]
                    if len(A1) * length_diff_c < len(A2) or len(
                        A2
                    ) * length_diff_c < len(A1):
                        pass  # don't compare if length of sequences differ to much
                    else:
                        todolist.append([A1, A2, y, z])
                        l += 1
...
def sort_genes():  # read the input file and sort sequences according to gene groups
    ...
    if args.histogram_only == True:  # if only histogram is wanted
        ...
        comparelist2 = list(
            set(([tuple(x) for sublist in comparelist2 for x in sublist]))
        )  # make list out of list with sublists
...
def sort_groups():  # read the gene groups and sort sequences to species level
    ...
    consensusfile = infile.replace(".fasta", "_consensussequences.fasta").replace(
        ".fastq", "_consensussequences.fasta"
    )  # total consensusfile
    try:  # remove  file if exists
    ...
    groupedseq = [
        int(x) for sublist in grouplist for x in sublist
    ]  # make flat list of numbers
    comparedseq = [x[3] for x in comparelist2]
    unique = set(comparedseq).difference(set(groupedseq))
    if len(unique) > 0:
        outputfile = os.path.join(
            outputfolder,
            infile.replace(".fastq", "_").replace(".fasta", "_") + "nogroup.group",
        )
        with open(outputfile, "a") as outputf:
            for y in unique:
                outputf.write(str(y) + "\n")
...
def sort(group_filename):
    ...
    if (
        len(grouplist) == 0
    ):  # if there are no groups, no need to compare rest of sequences
        print("--> Nothing to do")
...
def main():
    try:
        ...
        for infolder_file in infolder_file_list:
            ...
            tempfile = infile.replace(".fastq", "_compare.tmp").replace(
                ".fasta", "_compare.tmp"
            )
